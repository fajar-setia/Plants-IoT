...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="h-20 flex items-center justify-between px-5 border-b border-slate-700/50">
          <div className={`flex items-center gap-3 overflow-hidden transition-all duration-500 ${
            ...
            <div className="p-2.5 bg-linear-to-br from-teal-400 to-cyan-500 rounded-xl shadow-lg shadow-teal-500/30">
        ...
        <nav className="flex-1 px-3 py-6 space-y-2 overflow-y-auto overflow-x-hidden">
          {navItems.map(({ label, path, icon: Icon }) => (
            ...
            >
              <Icon size={22} className={`shrink-0 transition-transform duration-300 ${
                isActive(path) ? "scale-110" : "group-hover:scale-110"
              }`} />
              
              <span className={`font-medium whitespace-nowrap overflow-hidden transition-all duration-500 ${
                isOpen ? "opacity-100 w-auto" : "opacity-0 w-0"
              }`}>
                {label}
              </span>
        ...
        <div className="p-4 border-t border-slate-700/50">
          <div className={`flex items-center gap-3 transition-all duration-500 ${
            ...
            <div className="text-xs">
              <div className="text-slate-300 font-medium">All Systems Online</div>
      ...
      <div className={`transition-all duration-500 ease-in-out ${isOpen ? "ml-72" : "ml-20"}`} />
    </>
  );
}